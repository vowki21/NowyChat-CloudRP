...
AddEventHandler("chat:addMessage1", function(title, color, message, icon, color1, hex) 
  if(not chatVisibilityToggle)then
    if not hex then
      hex = "#FFFFFF"
    end
    ...
    SendNUIMessage({
      ...
      message = {
        color = color1,
        args = {title, message, color[1], color[2], color[3], icon, color1[1], color1[2], color1[3], hex},
        templateId = "default"
      }
    })
  end
